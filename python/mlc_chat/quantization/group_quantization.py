...
class GroupQuantize:  # pylint: disable=too-many-instance-attributes
    ...
    def __post_init__(self):
        ...
        self.linear_quant_axis = 0 if self.linear_weight_layout == "KN" else 1 # TODO
        ...
        return te.compute(
            ...
            fcompute=lambda *idx: tir.multiply(
                tir.subtract(
                    float_weight(*idx),
                    # tir_max_int,
                    zero(*idx[:axis], idx[axis] // self.group_size, *idx[axis + 1 :]),
                ),
    ...
    def quantize_weight(
        ...
        axis = self.linear_quant_axis # axis if axis >= 0 else len(weight.shape) + axis
    ...
    def _quantize(  # pylint: disable=too-many-locals
        ...
        zero = te.compute(
            scale_shape,
            lambda *idx: -min_value(*idx).astype(self.model_dtype) / scale(*idx),
            name="zero",
        )
...
class GroupQuantizeLinear(nn.Module):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self.config = config
        if self.config.linear_quant_axis == 0:
            num_group = tir.ceildiv(out_features, config.group_size)
        else:
        ...
        if config.linear_weight_layout == "KN":
            ...
            self.q_zero = nn.Parameter((num_group, out_features), config.model_dtype)
        else:
            if self.config.linear_quant_axis == 0:
                self.q_weight = nn.Parameter(
                    (config.num_storage_per_group * num_group, in_features), config.storage_dtype
                )
                self.q_scale = nn.Parameter((num_group, in_features), config.model_dtype)
                self.q_zero = nn.Parameter((num_group, in_features), config.model_dtype)
            else:
                self.q_weight = nn.Parameter(
